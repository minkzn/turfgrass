...
- `quiz.html` — 633-question quiz, 10 at a time
...
- Each question is modeled on the real Canvas quiz style for its module (verified against the instructor's actual M1-M5, M7, M8, and M9 quizzes) and sourced from that module's own course PDFs, with a curated slice of textbook enrichment on the same topics where it adds real depth.
